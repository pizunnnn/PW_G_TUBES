...
<div class="max-w-3xl mx-auto px-4">
    ...
    <div class="bg-white rounded-lg shadow p-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">
            <i class="fas fa-edit text-purple-600"></i> Edit Category
        </h1>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
            ...
            @if($category->icon)
                <div class="mb-6">
                    ...
                    <img src="{{ asset('storage/' . $category->icon) }}" alt="{{ $category->name }}" class="w-32 h-32 object-cover rounded">
                </div>
            @endif
            ...
            <div class="mb-6">
                <label for="icon" class="block text-gray-700 font-semibold mb-2">
                    <i class="fas fa-image"></i> Change Icon (jpg, png, max 2MB)
                </label>
                <input 
                    type="file" 
                    name="icon" 
                ...
                @error('icon')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                <p class="text-sm text-gray-500 mt-1">Leave empty to keep current icon</p>
