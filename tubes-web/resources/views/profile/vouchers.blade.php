...
<div class="max-w-6xl mx-auto px-4">
    <div class="mb-8">
        ...
        <h1 class="text-4xl font-bold text-gray-900 mb-2">
            <i class="fas fa-ticket text-purple-600"></i> My Vouchers
        </h1>
        <p class="text-gray-600">All your purchased voucher codes in one place</p>
    </div>
                        ...
                        <div class="flex items-center gap-4 mb-4">
                            <div class="w-16 h-16 bg-gradient-to-br from-purple-400 to-indigo-600 rounded-lg flex items-center justify-center flex-shrink-0">
                                @if($voucher->product->image)
                                    <img src="{{ asset('storage/' . $voucher->product->image) }}" 
                        ...
                        <div class="flex gap-2">
                            ...
                            @else
                                <button disabled class="flex-1 bg-gray-300 text-gray-600 py-2.5 rounded-lg font-semibold cursor-not-allowed">
                                    <i class="fas fa-check-circle"></i> Already Used
                                </button>
                            @endif
                            
                            <a href="{{ route('transactions.show', $voucher->transaction->transaction_code) }}" 
                        ...
                        <div class="mt-4 flex items-center justify-between text-sm">
                            @if($voucher->is_used)
                                <span class="text-gray-600">
                                    <i class="fas fa-circle-check text-green-600"></i> Used on {{ $voucher->used_at->format('d M Y') }}
                                </span>
                            @else
    ...
    @else
        <div class="bg-white rounded-xl shadow-lg p-12 text-center">
            ...
            <h3 class="text-xl font-semibold text-gray-700 mb-2">No vouchers yet</h3>
            <p class="text-gray-500 mb-6">Purchase products to get voucher codes</p>
